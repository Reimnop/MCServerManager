...
namespace MineStatLib
{
  ...
  public class MineStat
  {
    ...
    public MineStat(string address, ushort port, int timeout = DefaultTimeout,
      SlpProtocol protocol = SlpProtocol.Automatic)
    {
      Address = address;
      Port = port;
      Timeout = timeout;
      ...
      switch (protocol)
      {
        ...
        default:
          throw new ArgumentOutOfRangeException(nameof(protocol),
            "Invalid SLP protocol specified for parameter 'protocol'");
      }
    ...
    private ConnStatus ParseJsonProtocolPayload(byte[] rawPayload)
    {
      try
      {
        using var reader = new StreamReader(new MemoryStream(rawPayload), Encoding.Default);
        var root = JObject.ReadFrom(new JsonTextReader(reader));
        ...
        // Extract version
        Version = (string) root["version"]["name"];

        // the MOTD
        Motd = (string) root["description"]["text"];

        // the online player count
        CurrentPlayersInt = (int) root["players"]["online"];

        // the max player count
        MaximumPlayersInt = (int) root["players"]["max"];

        // the player list
        List<Player> players = new List<Player>();

        if (((JObject)root["players"]).ContainsKey("sample"))
        {
          foreach (JToken playerJson in root["players"]["sample"])
          {
            players.Add(new Player
            {
              Name = (string) playerJson["name"],
              Uuid = (string) playerJson["id"]
            });
          }
        }
